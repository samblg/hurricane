...
void JWordCountBolt::Prepare(std::shared_ptr<hurricane::collector::OutputCollector> outputCollector) {
    _outputCollector = outputCollector;
    _logFile = new std::ofstream("timestamp" + itos(rand()) + ".txt");
}

void JWordCountBolt::Cleanup() {
    delete _logFile;
}

std::vector<std::string> JWordCountBolt::DeclareFields() {
    return{ "word", "count" };
}

void JWordCountBolt::Execute(const hurricane::base::Tuple& tuple) {
